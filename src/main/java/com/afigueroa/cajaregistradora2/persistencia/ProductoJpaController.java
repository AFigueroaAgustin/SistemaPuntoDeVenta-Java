...
public class ProductoJpaController implements Serializable {
    ...
    public Producto findProductoByCodigo(String codigo) {
        EntityManager em = getEntityManager();
        try {
            // JPQL: Búsqueda personalizada donde 'p.codigo' coincide con el parámetro
            TypedQuery<Producto> q = em.createQuery(
                    "SELECT p FROM Producto p WHERE p.codigo = :codigoParam", Producto.class);
            
            q.setParameter("codigoParam", codigo);
            
            try {
                // Obtenemos un único resultado
                return q.getSingleResult();
            } catch (NoResultException ex) {
                // Si no se encuentra, devolvemos null (más fácil de manejar en ControLogica)
                return null; 
            }
        } finally {
